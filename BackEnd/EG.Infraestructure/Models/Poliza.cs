// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
#nullable disable
using System;
using System.Collections.Generic;

namespace EG.Infraestructure.Models;

public partial class Poliza
{
    public int PkidPoliza { get; set; }

    public int FkidAnioSis { get; set; }

    public int FkidMesSis { get; set; }

    public int FkidTipoPolizaSis { get; set; }

    public string ClavePoliza { get; set; }

    public string NombrePoliza { get; set; }

    public DateTime FechaPoliza { get; set; }

    public bool EstaBalanceado { get; set; }

    public bool Activo { get; set; }

    public DateTime? FechaCreacion { get; set; }

    public int UsuarioCreacion { get; set; }

    public DateTime? FechaModificacion { get; set; }

    public int? UsuarioModificacion { get; set; }

    public bool? PermitirModificar { get; set; }

    public int? FkidAccionAutorizarSis { get; set; }

    public bool? Autorizado { get; set; }

    public DateTime? FechaSolicitud { get; set; }

    public DateTime? FechaAutorizacion { get; set; }

    public byte[] RowVersion { get; set; }

    public virtual ICollection<Baja> Bajas { get; set; } = new List<Baja>();

    public virtual ICollection<Cheque> Cheques { get; set; } = new List<Cheque>();

    public virtual ICollection<Clc> Clcs { get; set; } = new List<Clc>();

    public virtual ICollection<Contrato1> Contrato1s { get; set; } = new List<Contrato1>();

    public virtual ICollection<EgresoAutorizado> EgresoAutorizados { get; set; } = new List<EgresoAutorizado>();

    public virtual ICollection<Factura> Facturas { get; set; } = new List<Factura>();

    public virtual Anio FkidAnioSisNavigation { get; set; }

    public virtual TipoPoliza FkidTipoPolizaSisNavigation { get; set; }

    public virtual ICollection<IngreAdecuacion> IngreAdecuacions { get; set; } = new List<IngreAdecuacion>();

    public virtual ICollection<IngresoAutorizado> IngresoAutorizados { get; set; } = new List<IngresoAutorizado>();

    public virtual ICollection<OrdenCompra> OrdenCompras { get; set; } = new List<OrdenCompra>();

    public virtual ICollection<PolizaDetalle> PolizaDetalles { get; set; } = new List<PolizaDetalle>();

    public virtual ICollection<RegresoChequeSuficienciaPoliza> RegresoChequeSuficienciaPolizaFkidPolizaOriginalContaNavigations { get; set; } = new List<RegresoChequeSuficienciaPoliza>();

    public virtual RegresoChequeSuficienciaPoliza RegresoChequeSuficienciaPolizaFkidPolizaReversionContaNavigation { get; set; }

    public virtual ICollection<SolicitudSalidum> SolicitudSalida { get; set; } = new List<SolicitudSalidum>();

    public virtual Usuario UsuarioCreacionNavigation { get; set; }

    public virtual Usuario UsuarioModificacionNavigation { get; set; }
}