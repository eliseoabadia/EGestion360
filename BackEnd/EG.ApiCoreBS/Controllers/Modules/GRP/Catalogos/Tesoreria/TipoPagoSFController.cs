...
namespace EG.ApiCoreBS.Controllers.Catalogos.Tesoreria
{
    ...
    public class TipoPagoSFController : EG.ApiCoreBS.Controllers.BaseApiController
    {
        private readonly ITipoPagoSFService _service;

        public TipoPagoSFController(ITipoPagoSFService service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Create([FromBody] TipoPagoSFDto dto)
        {
            ...
            return CreatedAtAction(nameof(GetById), new { id = response.PkidTipoPagoSf }, response);
        }
