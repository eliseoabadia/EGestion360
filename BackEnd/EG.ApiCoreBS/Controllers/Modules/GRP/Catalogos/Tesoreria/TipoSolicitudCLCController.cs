...
namespace EG.ApiCoreBS.Controllers.Catalogos.Tesoreria
{
    ...
    public class TipoSolicitudCLCController : EG.ApiCoreBS.Controllers.BaseApiController
    {
        private readonly ITipoSolicitudCLCService _service;

        public TipoSolicitudCLCController(ITipoSolicitudCLCService service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Create([FromBody] TipoSolicitudCLCDto dto)
        {
            ...
            return CreatedAtAction(nameof(GetById), new { id = response.PkidTipoSolicitudClc }, response);
        }
