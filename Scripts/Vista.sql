...
WHERE a.Activo = 1;
GO
PRINT N'Creando Vista [TES].[Vw_Banco]...';
...
WHERE b.Activo = 1;
GO
PRINT N'Creando Vista [TES].[Vw_TipoPagoSF]...';
...
WHERE tpsf.Activo = 1;
GO
PRINT N'Creando Vista [TES].[Vw_TipoInversion]...';
...
WHERE ti.Activo = 1;
GO
PRINT N'Creando Vista [TES].[Vw_TipoPlazo]...';
...
WHERE tp.Activo = 1;
GO
PRINT N'Creando Vista [TES].[Vw_TipoRetiro]...';
...
WHERE tr.Activo = 1;
GO
PRINT N'Creando Vista [TES].[Vw_TipoSolicitudCLC]...';
...
WHERE ts.Activo = 1;
GO
PRINT N'Creando Vista [TES].[Vw_TipoPago]...';
...
WHERE tp.Activo = 1;
GO
PRINT N'Creando Vista [TES].[Vw_TipoMoneda]...';


GO

CREATE OR ALTER VIEW [TES].[Vw_TipoMoneda] AS
SELECT
    tm.PKIdTipoMoneda,
    tm.FKIdPais_SIS,
    tm.Descripcion,
    ...
    pais.CodigoISO2 AS PaisCodigoISO2,
    CONCAT(tm.CodigoISO4217, ' - ', tm.Descripcion) AS ClaveNombre
FROM TES.TipoMoneda tm
LEFT JOIN SIS.Paises pais ON tm.FKIdPais_SIS = pais.PKIdPais AND pais.Activo = 1
WHERE tm.Activo = 1;
GO
...
SELECT
    ...
    pais.Nombre AS PaisNombre,
    CONCAT(tm.CodigoISO4217, ' - ', CONVERT(NVARCHAR(30), tc.Fecha, 23)) AS ClaveNombre
FROM TES.TipoCambio tc
LEFT JOIN TES.TipoMoneda tm ON tc.FKIdTipoMoneda_TES = tm.PKIdTipoMoneda AND tm.Activo = 1
LEFT JOIN SIS.Paises pais ON tm.FKIdPais_SIS = pais.PKIdPais AND pais.Activo = 1
WHERE tc.Activo = 1;
GO
PRINT N'Creando Vista [TES].[Vw_IntermediarioFinanciero]...';
...
WHERE i.Activo = 1;
GO
PRINT N'Creando Vista [TES].[Vw_Instrumento]...';
...
WHERE ins.Activo = 1;
GO
PRINT N'Creando Vista [TES].[Vw_CuentaBancaria]...';
